...
import { createActivity } from "../../api/HealthAPI";

export default function AddActivityModal() {
  const navigate = useNavigate();
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const modalExpense = queryParams.get("newActivity");
    ...
    if (action === "add activity") {
      mutate(formData, {
        onSuccess: () => {
          ...
          reset(), toast.success("Activity added");
          navigate(location.pathname, { replace: true });
        },
      });
    } else if (action === "add other activity") {
      mutate(formData, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ["latestActivities"] });
          queryClient.invalidateQueries({ queryKey: ["latestFoodModal"] });
          reset(), toast.success("Activity added, you can add another one");
        },
      });
    }
  };
              ...
              >
                <Dialog.Panel className="w-1/2 max-w-4xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-xl transition-all p-16">
                  ...
                  <p className="text-xl text-center">
                    ...
                    <span className="text-nice-red">add the activity</span>
                  </p>
                  <form
